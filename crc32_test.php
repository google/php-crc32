...
// Do the crcs produce the correct hashs
foreach ($crcs as $crc_class) {
    foreach ($algos as $i => $poly) {
        $ok = true;
        $test_name = "$crc_class(" . CRC32::string($poly) . ")";

        if (!$crc_class::supports($poly)) {
            echo "SKIP: $test_name unsupported polynomial\n";
            continue;
        }

        $crc = new $crc_class($poly);
        foreach ($tests as $input => $answers) {
            $expected = $answers[$i];
            ...
            if ($actual !== $expected) {
                $ok = false;
                echo "FAIL: $test_name.hash_extend(" . var_export($input, true) . ") = $actual wanted $expected\n";
            }
...
foreach ($crcs as $crc_class) {
    foreach ($algos as $i => $poly) {
        ...
        if (!$crc_class::supports($poly)) {
            echo "SKIP: $test_name clone unsupported polynomial\n";
            continue;
        }
        ...
        $actual = $a->hash();
        $expected = $tests['abc'][$i];
        if ($actual !== $expected) {
            $ok = false;
            echo "FAIL: $test_name.clone(a).update('abc') = $actual wanted $expected\n";
            continue;
        }

        $actual = $b->hash();
        $expected = $tests['abcdefgh'][$i];
        if ($actual !== $expected) {
            $ok = false;
            echo "FAIL: $test_name.clone(b).update('abcdefgh') = $actual wanted $expected\n";
            continue;
        }
    }
}
